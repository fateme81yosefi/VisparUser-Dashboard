...
function useLoginOTP() {
  ...
  const loginOTPCode = async (code: number) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        switch (response.status) {
          case 425:
            setErrorOTPCodeVerify("برای درخواست مجدد کد 3 دقیقه صبر کنید");

            break;

          case 422:
            setErrorOTPCodeVerify("خطایی رخ داده لطفا مجددا تلاش کنید");
  ...
  return { errorOTP,er loggedInOTP,otpCodeVerify, loginOTP ,loginOTPCode};
}
